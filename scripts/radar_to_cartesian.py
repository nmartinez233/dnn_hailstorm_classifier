...
def convert(file_name):
    ...
    filename = grid_file_directory+file_name[33:(len(file_name)-4)]+".nc"
...
if __name__ == "__main__":
    for j in range(3,4):
        i = 0
        file_directory = '../data/used_NEXRAD_data/cluster%d/*V06*' % (j)
        file_list = glob.glob(file_directory)
        grid_file_directory = '../data/netCDF4/cluster%d' % (j)
